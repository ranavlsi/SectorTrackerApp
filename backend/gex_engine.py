...
def get_gex_profile(ticker: str):
    """Calculates Gamma Exposure profile for a specific ticker on demand."""
    try:
        t = yf.Ticker(ticker)
        ...
        if not options:
            return {"error": "No options chain available for this ticker"}
        ...
        for strike in strikes:
            ...
            if not put_row.empty:
                ...
                if pd.notna(iv) and pd.notna(p_oi):  # FIXED BUG HERE (was checking c_oi)
